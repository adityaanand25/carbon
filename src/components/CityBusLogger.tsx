...
interface CityBusLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function CityBusLogger({ onClose, onLog }: CityBusLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const busData = activityData['City Bus'];

    if (!busData) {
      alert('City Bus data not found.');
      return;
    }

    const energyConsumedKcal = busData.energy * distanceKm;
    const co2EmissionsGrams = busData.co2 * distanceKm;

    const replacedVehicleData = vehicleData[
      replacedVehicle as keyof typeof vehicleData
    ];
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log City Bus Trip</h2>
        <div className="mb-4">
