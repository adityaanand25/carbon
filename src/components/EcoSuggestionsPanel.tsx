...
import { CheckCircle, Circle, ArrowRight, TrendingDown, Lightbulb, Star, Calendar } from 'lucide-react';
import * as Icons from 'lucide-react';
import { Recommendation, CalendarDay } from '../types';
...
export default function EcoSuggestionsPanel({ recommendations, onCompleteRecommendation, calendarData = [], dailyGoal = 10 }: EcoSuggestionsPanelProps) {
  const [filter, setFilter] = useState<'all' | 'easy' | 'medium' | 'hard'>('all');
  const [sortBy, setSortBy] = useState<'impact' | 'difficulty'>('impact');
  const [activeTab, setActiveTab] = useState<'suggestions' | 'calendar'>('suggestions');
  ...
  return (
    <div className="h-[580px] bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-white/50 flex flex-col">
      {/* Header with Tabs */}
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Track progress and view your carbon calendar
            </p>
          </div>
        ...
        {activeTab === 'suggestions' ? (
          ...
          <div className="h-full p-6">
            <CarbonCalendar
              ...
              dailyGoal={dailyGoal}
              onDateSelect={(date: string) => console.log('Selected date:', date)}
            />
          </div>
        )}
      </div>
