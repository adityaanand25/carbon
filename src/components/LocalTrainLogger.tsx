...
interface LocalTrainLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function LocalTrainLogger({ onClose, onLog }: LocalTrainLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const trainData = activityData['Local Train'];

    if (!trainData) {
      alert('Local Train data not found.');
      return;
    }

    const energyConsumedKcal = trainData.energy * distanceKm;
    const co2EmissionsGrams = trainData.co2 * distanceKm;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log Local Train Trip</h2>
        <div className="mb-4">
