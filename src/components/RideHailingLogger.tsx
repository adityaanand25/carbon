...
interface RideHailingLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function RideHailingLogger({ onClose, onLog }: RideHailingLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const rideHailingData = activityData['Ola/Uber/Rapido'];

    if (!rideHailingData) {
      alert('Ola/Uber/Rapido data not found.');
      return;
    }

    const energyConsumedKcal = rideHailingData.energy * distanceKm;
    const co2EmissionsGrams = rideHailingData.co2 * distanceKm;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log Ola/Uber/Rapido Trip</h2>
        <div className="mb-4">
