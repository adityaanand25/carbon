...
interface ElectricTaxiLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function ElectricTaxiLogger({ onClose, onLog }: ElectricTaxiLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const taxiData = activityData['Electric Taxi'];

    if (!taxiData) {
      alert('Electric Taxi data not found.');
      return;
    }

    const energyConsumedKcal = taxiData.energy * distanceKm;
    const co2EmissionsGrams = taxiData.co2 * distanceKm;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log Electric Taxi Trip</h2>
        <div className="mb-4">
