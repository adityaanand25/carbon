...
interface TaxiLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function TaxiLogger({ onClose, onLog }: TaxiLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const taxiData = activityData['Taxi (Petrol/Diesel)'];

    if (!taxiData) {
      alert('Taxi (Petrol/Diesel) data not found.');
      return;
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log Taxi (Petrol/Diesel) Trip</h2>
        <div className="mb-4">
