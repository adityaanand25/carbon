...
interface TramLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function TramLogger({ onClose, onLog }: TramLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const tramData = activityData['Tram/Light Rail'];

    if (!tramData) {
      alert('Tram/Light Rail data not found.');
      return;
    }

    const energyConsumedKcal = tramData.energy * distanceKm;
    const co2EmissionsGrams = tramData.co2 * distanceKm;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log Tram/Light Rail Trip</h2>
        <div className="mb-4">
