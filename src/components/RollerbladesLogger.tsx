...
interface RollerbladesLoggerProps {
  onClose: () => void;
...
// Assuming rollerblading has slightly higher energy use and food-based emissions than walking
const rollerbladeData = { co2: 25, energy: 0.4 };

export default function RollerbladesLogger({ onClose, onLog }: RollerbladesLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const calculatedSavings = Object.entries(vehicleData).map(([vehicle, data]) => {
      const co2Saved = dist * (data.co2 - rollerbladeData.co2);
      const energySaved = dist * (data.energy - rollerbladeData.energy);
      return {
        vehicle,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="bg-gradient-to-br from-white/90 to-slate-100/90 rounded-3xl shadow-2xl p-8 border border-white/50 w-full max-w-2xl transform transition-all duration-300">
        <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Log Rollerblading Activity 🛼</h2>
        ...
        {savings && (
          <div className="mt-8 p-6 bg-white/70 rounded-2xl border border-white/30">
            <h3 className="text-xl font-bold text-gray-800 mb-4 text-center">
              Savings for {savings.distance} km of Rollerblading
            </h3>
