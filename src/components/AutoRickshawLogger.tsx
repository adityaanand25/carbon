...
interface AutoRickshawLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function AutoRickshawLogger({ onClose, onLog }: AutoRickshawLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const autoRickshawData = activityData['Auto-Rickshaw'];

    if (!autoRickshawData) {
      alert('Auto-Rickshaw data not found.');
      return;
    }

    const energyConsumedKcal = autoRickshawData.energy * distanceKm;
    const co2EmissionsGrams = autoRickshawData.co2 * distanceKm;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log Auto-Rickshaw Trip</h2>
        <div className="mb-4">
