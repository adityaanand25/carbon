...
interface CycleRickshawLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function CycleRickshawLogger({ onClose, onLog }: CycleRickshawLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const cycleRickshawData = activityData['Cycle-Rickshaw'];

    if (!cycleRickshawData) {
      alert('Cycle-Rickshaw data not found.');
      return;
    }

    const energyConsumedKcal = cycleRickshawData.energy * distanceKm;
    const co2EmissionsGrams = cycleRickshawData.co2 * distanceKm;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log Cycle-Rickshaw Trip</h2>
        <div className="mb-4">
