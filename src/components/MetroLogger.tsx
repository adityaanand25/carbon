...
interface MetroLoggerProps {
  onClose: () => void;
  onLog: (logData: any) => void;
}

export default function MetroLogger({ onClose, onLog }: MetroLoggerProps) {
  ...
  const calculateSavings = () => {
    ...
    const metroData = activityData['Metro/Subway'];

    if (!metroData) {
      alert('Metro/Subway data not found.');
      return;
    }

    const energyConsumedKcal = metroData.energy * distanceKm;
    const co2EmissionsGrams = metroData.co2 * distanceKm;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Log Metro/Subway Trip</h2>
        <div className="mb-4">
